...
def read_rs_log(path):
    ...
    for log in run_log_list:
        ...
        for line in range(total_count-1,total_count):
            ...
            date,interval,total_iops,total_resp,cpu_total,cpu_sys,rwmix_tmp,read_iops,read_resp,write_iops,write_resp,read_bw,write_bw,total_bw,r_size,mkdir_rate,mkdir_resp,rmdir_rate,rmdir_resp,create_rate,create_resp,open_rate,open_resp,close_rate,close_resp,delete_rate,delete_resp=line_value.rstrip('\r\n').split()
        ...
        rs=(task_name,client_num,size,run_type,thread,mode,rwmix,dev,read_iops,write_iops,total_iops,read_bw,write_bw,total_bw,read_resp,write_resp,total_resp)
        rs_list.append(rs)
...
def insert_value(rs_list):
    for rs in rs_list:
        ...
        rs_str='\",\"'.join(rs)
        rs_str='\"'+ rs_str +'\"'
        sql_str='insert into "main".vdbench_rs_table(task_name,client_num,r_size,run_type,threads,mode,rwmix,dev,read_iops,write_iops,total_iops,read_bw,write_bw,total_bw,read_resp,write_resp,total_resp) values(%s);' % rs_str 
        cur.execute(sql_str)
    conn.commit()
...
def create_report_html(dir_path):
    ...
    iops_title_list=['client_num','r_size','threads','mode','rwmix','read_iops','write_iops','total_iops','total_resp']
    for iops in iops_title_list:
        f.write('<th class="red" >%s</th>\n' % iops)
    f.write('</tr>\n')

    sql_str = 'select client_num,r_size,threads,mode,rwmix,read_iops,write_iops,total_iops,total_resp from "main".vdbench_rs_table'
    sql_rs = cur.execute(sql_str)
    for row in sql_rs:
        f.write('<tr>\n')
        rs_list = []
    ...
    f.write('<table cellpadding="0" id="iops_table" >\n')
    f.write('<tr class="top" >\n')
    iops_title_list=['client_num','r_size','threads','mode','rwmix','read_bw','write_bw','total_bw','total_resp']
    for iops in iops_title_list:
        f.write('<th class="red" >%s</th>\n' % iops)
    f.write('</tr>\n')

    sql_str = 'select client_num,r_size,threads,mode,rwmix,read_bw,write_bw,total_bw,total_resp from "main".vdbench_rs_table'
    sql_rs = cur.execute(sql_str)
    for row in sql_rs:
        f.write('<tr>\n')
        rs_list = []
