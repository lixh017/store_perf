...
run_type='para,seq,all'
#iodepth='1,4,8,16,32'
iodepth='1,2,4,8,16,32'
#numjobs='1,4,8,16,32'
numjobs='1,2,4,8,16,32'
rw_mix='50,60,70,80,90'
file_size=0
...
do
    ...
    do
        ...
        do
            ...
            for depth in `echo $iodepth|tr ',' ' '`
            do
                for job in `echo $numjobs|tr ',' ' '`
                do
                    ...
                    do
                        temp_file=${temp_dir}/${rs}/`printf "%0*d" "4" ${flg}`_${rs}_${job}jobs_${depth}iodepth_${run}_${mix}rw_${mode}.t
                        if [ -f ${temp_file} ]
                        then
...
cat > ${temp_file} << EOF 
#!/bin/bash
# fio test: ${temp_file} ,v 1.0 `date` $
desc="====fio test on [ ${rs}-${job}jobs_${depth}iodepth_${run}_${mix}rw_${mode} ] ===="
...
fio_conf=\`/bin/bash \${dir}/../../genfio.sh -d \${test_file} \${cache_falg} \${engine_flag} ${run_flag} -b ${rs} \${size_flag} -m ${mode} \${run_time_flag} \${share_flag} -D ${depth} -n ${job} -M ${mix} -x \${run_result_dir}/\`

echo \$fio_conf |grep -q '.fio$' 
if [ \$? -ne 0 ]
then
    echo "genfio conf failed!"
    exit 3
fi
rs_name=\`basename \$fio_conf|cut -d '.' -f 1\`
...
if [ -z \${host_list} ]
then
    mkdir -p \${run_result_dir}/\${rs_name}/logs/localhost
    expect_success fio \$fio_conf --output=\`pwd\`/${run_result_dir}/\${rs_name}_run.log
    eval mv "/tmp/${rs}-${depth}iodepth-${job}jobs-*-${mode}-${mix}rw*.log" "\${run_result_dir}/\${rs_name}/logs/localhost"
else
    >./host.list
    ...
    expect_success fio --client=./host.list \$fio_conf --output=\`pwd\`/\${run_result_dir}/\${rs_name}_run.log 
    for host in \`echo \${host_list}|tr ',' ' '\`
    do
        mkdir -p \${run_result_dir}/\${rs_name}/logs/\${host}
        eval mv "/tmp/${rs}-${depth}iodepth-${job}jobs-*-${mode}-${mix}rw*.log.\${host}" "\${run_result_dir}/\${rs_name}/logs/\${host}"
    done
fi
EOF
